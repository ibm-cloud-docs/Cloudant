...
### Eliminating conflicts using replication
...
replications are different than those generated by default, so it is
...
`use_bulk_get`
:   Set to `true` to improve replication job performance. A replication fetches documents in batches from the source. The increased replication rate can consume the source and target account capacity quicker, and result in concurrent non-replication requests on those accounts to experience more
429 HTTP errors.
