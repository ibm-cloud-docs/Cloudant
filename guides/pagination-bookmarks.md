...
First, let's perform a search for all the cities in the US. We're using [{{site.data.keyword.cloudant_short_notm}} Query](https://cloud.ibm.com/docs/Cloudant?topic=cloudant-query), so the operation is specified as a block of JSON:
...
No. MapReduce views do not accept a `bookmark`. Use the [skip and limit](https://cloud.ibm.com/docs/Cloudant?topic=cloudant-using-views) to page through results.
