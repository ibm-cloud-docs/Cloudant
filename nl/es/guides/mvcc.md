...
es el método que utilizan las bases de datos de {{site.data.keyword.cloudantfull}} para garantizar que todos los nodos de un clúster de bases de datos contienen solo la [versión más reciente](../api/document.html) de un documento.
{:shortdesc}

Puesto que las bases de datos de {{site.data.keyword.cloudant_short_notm}} son [finalmente coherentes](cap_theorem.html), se necesita este sistema para evitar que se produzcan incoherencias entre nodos como resultado de la sincronización entre documentos obsoletos.
...
Para buscar conflictos, añada el parámetro de consulta [`conflicts=true`](../api/database.html#get-changes) al recuperar un documento.
