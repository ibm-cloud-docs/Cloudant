...
Devi specificare il valore `_rev` precedente durante l'[aggiornamento di un documento](../api/document.html#update)
altrimenti la tua richiesta non riesce e restituisce un [errore 409](../api/http.html#409).

>   **Nota**: `_rev` non deve essere utilizzato per creare un sistema di controllo della versione.
    Il motivo è che questo è un valore interno utilizzato dal server.
    Inoltre,
...
una soluzione è quella di [creare un nuovo documento](../api/document.html#documentCreate) per ogni revisione.
...
aggiungi il parametro della query [`conflicts=true`](../api/database.html#get-changes) durante il recupero di un documento.
...
Per un esempio pratico di come implementare una unione delle modifiche,
vedi [questo progetto con il codice di esempio ![Icona link esterno](../images/launch-glyph.svg "Icona link esterno")](https://github.com/glynnbird/deconflict){:new_window}.
