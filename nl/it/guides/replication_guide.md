...
_Alla fine_,
le copie del database completano la propria replica
in modo che tutte le modifiche a un documento siano presenti in ciascuna copia.
...
[`"use_checkpoints": false`](../api/replication.html#checkpoints) durante la richiesta della replica.
...
Le credenziali di accesso fornite nei parametri di origine e di destinazione non richiedono diritti di amministratore completi.
...
In generale,
[crea chiavi API](../api/authorization.html#creating-api-keys) che abbiano:
...
passa un [argomento `since`](../api/database.html#the-since-argument) con il numero di sequenza da cui vuoi iniziare.
...
Sono [disponibili](../api/advanced_replication.html) opzioni avanzate dell'API di replica,
che consentono un aumento o una diminuzione della quantità di potenza di calcolo utilizzata durante la replica.
Ad esempio:
