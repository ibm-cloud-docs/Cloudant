...
Vous trouverez ci-après une présentation de [IAM ![Icône de lien externe](../images/launch-glyph.svg "Icône de lien externe")](https://console.bluemix.net/docs/iam/index.html#iamoverview){:new_window}, concernant notamment :
...
Le contrôle d'accès existant de {{site.data.keyword.cloudant_short_notm}} possède également un concept de clés d'API, et lorsqu'on parle de données d'identification existantes de {{site.data.keyword.cloudant_short_notm}} ou de combinaisons nom d'utilisateur/mot de passe, il est également toujours question des clés d'API {{site.data.keyword.cloudant_short_notm}}.
...
- `username` : Nom de service de l'utilisateur d'instance {{site.data.keyword.cloudant_short_notm}} dans l'URL.
...
<td headers="advantages" valign="top"><ul><li>Gestion des accès pour plusieurs services à l'aide d'une seule interface. L'accès à un utilisateur peut être révoqué globalement.
<li>Clés d'API au niveau du compte via les ID de service.
...
<td headers="disadvantages"><ul><li>Si vous n'utilisez pas les bibliothèques prises en charge par {{site.data.keyword.cloudant_short_notm}}, il est possible que des modifications soient nécessaires au niveau de l'application pour utiliser les clés d'API d'IAM et les jetons d'accès.<li>Aucune modification au niveau de la base de données (pour l'instant).</li>
...
<ul><li>Pas de clé d'API au niveau du compte : vous devez utiliser les données d'identification `root` pour gérer les bases de données.
...
{{site.data.keyword.cloud_notm}} IAM exige qu'une clé d'API IAM soit échangée contre un jeton d'accès limité dans le temps avant d'envoyer une demande à une ressource ou à un service. Le jeton d'accès est ensuite inclus dans l'en-tête HTTP `Authorization` envoyée au service. Lorsque le jeton d'accès expire, le client doit en obtenir un autre du service de jeton IAM. Pour plus d'informations, veuillez consulter la documentation [Getting an {{site.data.keyword.cloud_notm}} IAM token by using an API key ![Icône de lien externe](../images/launch-glyph.svg "Icône de lien externe")](https://console.bluemix.net/docs/iam/apikey_iamtoken.html#iamtoken_from_apikey).
...
Si vous avez des difficultés à utiliser IAM pour vous authentifier lorsque vous envoyez des demandes à votre instance de service {{site.data.keyword.cloudant_short_notm}}, vérifiez les points suivants.

### Vérifiez que votre compte est activé pour IAM

Vous devez émettre un ticket de demande de service pour confirmer qu'une instance de service a été activée pour IAM.
