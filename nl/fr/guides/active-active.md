...
> **Remarque** : Pour un déploiement actif-actif,
...
[Créez les bases de données](../api/database.html#create) que vous voulez répliquer entre chaque compte.
...
Il est intéressant d'utiliser une [clé d'API](../api/authorization.html#api-keys) pour les réplications continues.
...
> **Remarque** : Notez soigneusement le mot de passe.
  Il est en effet impossible de le récupérer ultérieurement.

### Etape 3 : Octroi des droits d'accès

[Accordez à la clé d'API](../api/authorization.html#modifying-permissions) un droit d'accès en lecture et en écriture sur les deux bases de données.

Si vous voulez également répliquer les index, attribuez des droits d'administration.

Utilisez le tableau de bord de {{site.data.keyword.cloudant_short_notm}} ou consultez les informations d'[autorisation](../api/authorization.html) pour savoir comment accorder les droits à l'aide d'un programme.
...
> **Remarque** : Si cette étape échoue car la base de données `_replicator` n'existe pas, créez-la.
...
Par exemple, une simple demande `GET` envoyée au noeud final de la base de données principale doit normalement renvoyer des [détails sur la base de données](../api/database.html#getting-database-details).
...
* Envisagez la surveillance des [réplications](../api/advanced_replication.html) entre les bases de données.
...
> **Remarque** : Si vous implémentez le réacheminement ou le basculement suite à un test d'intégrité, vous pouvez ajouter les vérifications correspondantes afin d'éviter des réacheminements prématurés vers une instance de service toujours en cours de récupération.
...
Vous pouvez obtenir plus de renseignements sur la [surveillance de l'état de réplication](../api/advanced_replication.html#replication-status).

> **Remarque** : Si une base de données reçoit sans cesse des modifications, il est peu probable que l'état de réplication soit 0.
  Vous devez décider du seuil de statut acceptable ou représentant un état d'erreur.

### Index

* Les index sont-ils suffisamment à jour ?
  Vérifiez-les à l'aide des noeuds finaux des [tâches actives](../api/active_tasks.html).
