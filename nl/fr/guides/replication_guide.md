...
Les données peuvent être copiées entre différentes bases de données dans le même compte Cloudant ainsi
que dans des comptes et des centres de données différents.
{:shortdesc}

Les données peuvent même être répliquées entre un compte Cloudant et un appareil mobile en utilisant [Cloudant Sync ![External link icon](../images/launch-glyph.svg "External link icon")](https://cloudant.com/product/cloudant-features/sync/){:new_window}
ou [PouchDB ![External link icon](../images/launch-glyph.svg "External link icon")](http://pouchdb.com/){:new_window}.
...
Le protocole de réplication de Cloudant est compatible avec plusieurs autres bases de données et bibliothèques,
ce qui fait qu'il est totalement adapté à IoT (Internet of Things) et aux applications mobiles.

Ce guide présente les fonctions de réplication de Cloudant,
décrit les cas d'utilisation communs et indique comment effectuer
la réplication de votre application.

## Qu'est-ce que la réplication ?

Cloudant est un magasin de données JSON réparti incluant une API HTTP.
Cloudant peut être exécuté en tant que service sur plusieurs clouds,
ou dans votre armoire de serveurs.
Les documents sont stockés dans les base de données et peuvent atteindre n'importe quelle taille car Cloudant fragmente ses données dans un grand nombre de noeuds.
La réplication est le processus de copie de données entre une base de données source et une base de données cible.
Il n'est pas nécessaire que ces bases de données se trouvent sur le même compte Cloudant,
...
Le tableau de bord Cloudant inclut une interface permettant de déclencher la réplication.
Ouvrez l'onglet Replication du tableau de bord Cloudant puis cliquez sur le bouton `New Replication`.
...
## Comment exécuter la réplication dans différents comptes Cloudant ?

La source et la cible d'une réplication sont les URL des bases de données Cloudant,
...
## Comment démarrer la réplication en utilisant l'API Cloudant ?

Chaque compte Cloudant a une base de données spéciale appelée `_replicator`,
...
Zone           | Objectif
----------------|--------
`_id`           | Le fait de renseigner une zone `_id` est facultatif mais cela peut être utile pour l'identification des tâches de réplication. Cloudant génère une valeur pour vous si vous n'en indiquez pas.
`source`        | URL de la base de données Cloudant source, incluant les données d'identification de connexion.
`target`        | URL de la base de données Cloudant cible, incluant les données d'identification de connexion.
...
Toutefois,
le fait que les bases de données Cloudant soient réparties
...
Cloudant utilise un modèle de 'cohérence finale'.
...
après un moment défini. Cela est dû au fait que la liste de modifications peut être fournie par une autre copie de base de données,
...
Cloudant en utilisant le mécanisme de [point de contrôle](#checkpoints) qui
...
être ['idempotent' ![External link icon](../images/launch-glyph.svg "External link icon")](http://www.eaipatterns.com/IdempotentReceiver.html){:new_window}.
...
Cloudant dispose d'un droit utilisateur `_replicator` spécial.
...
Les clés d'API peuvent être créées et configurées dans le tableau de bord Cloudant,
pour chaque base de données.

![réplication](../images/replication_guide_5.png)

Il est également possible de les créer [à l'aide d'un programme](../api/authorization.html#creating-api-keys) en utilisant l'API Cloudant.
...
"`Make this replication continuous`" lorsque vous définissez une tâche de réplication dans le tableau de bord Cloudant,
ou en définissant l'indicateur ["`continuous`"](../api/replication.html#checkpoints) dans l'API Cloudant.
...
Vous pouvez vérifier le statut de la base de données `_replicator` de Cloudant à tout moment,
...
De plus,
Le noeud final `/_active_tasks` du compte Cloudant peut être utilisé afin de voir la progression de la réplication.
...
La réplication ne concerne pas uniquement le transfert de données de Cloudant vers Cloudant.
Le protocole de réplication de Cloudant est compatible avec d'autres bases de données et bibliothèques pour différentes applications du monde réel.

### Apache CouchDB

[Apache CouchDB ![External link icon](../images/launch-glyph.svg "External link icon")](http://couchdb.apache.org/){:new_window} est une base de données à source ouverte
pouvant communiquer avec Cloudant,
et nécessitant une configuration minimale.
Les applications incluent les fonctions suivantes :

-   Sauvegarde : Répliquez vos données à partir de Cloudant vers vos propres bases de données CouchDB
    ...
    [Amazon Glacier ![External link icon](../images/launch-glyph.svg "External link icon")](https://aws.amazon.com/glacier/){:new_window}.
-   Collecte de données locales en priorité : Placez tout d'abord vos données sur la version locale d'Apache CouchDB,
    puis répliquez-les dans Cloudant pour un stockage à long terme, l'agrégation
    et l'analyse.

### PouchDB

[PouchDB ![External link icon](../images/launch-glyph.svg "External link icon")](http://pouchdb.com/){:new_window} est une base de données
sur navigateur à source ouverte qui permet la réplication bidirectionnelle des données entre le navigateur et Cloudant.
Le stockage des données dans un navigateur Web au niveau client permet aux applications Web de fonctionner,
même sans connexion Internet.
PouchDB peut synchroniser les données modifiées depuis ou vers Cloudant lorsqu'une connexion Internet est disponible.
...
[CloudantSync ![External link icon](../images/launch-glyph.svg "External link icon")](https://cloudant.com/cloudant-sync-resources/){:new_window} est un ensemble de bibliothèques
pour iOS et Android permettant aux données d'être stockées localement sur un appareil mobile
et synchronisées avec Cloudant lorsque la connectivité mobile le permet.
...
où l'état de l'application est conservé dans Cloudant par réplication,
...
La réplication filtrée de Cloudant permet la définition d'une fonction JavaScript utilisant la valeur de retour
...
Par exemple,
utilisez la [bibliothèque Node.js de Cloudant](../libraries/supported.html#node-js)
...
>   **Remarque** : L'ordre des documents dans le flux `_changes` n'est pas toujours le même.
    ...
    Cela est dû au fait que les données sont renvoyés à partir de plusieurs noeuds Cloudant et
...
Les clés d'API sont générées dans le tableau de bord Cloudant ou [via l'API](../api/authorization.html#creating-api-keys).
Chaque clé peut disposer de droits individuels concernant une base de données Cloudant spécifique.
Cloudant doit pouvoir écrire ses documents de point de contrôle à la fin de la "lecture" de la réplication.
...
contactez l'[équipe de support IBM Cloudant ![External link icon](../images/launch-glyph.svg "External link icon")](mailto:support@cloudant.com){:new_window} pour obtenir des conseils supplémentaires.
...
Cloudant ne vous empêche donc pas de créer des processus de réplication supplémentaires.
Toutefois, chaque tâche de réplication utilise des ressources système.

Vous pouvez vérifier les "réplications actives" dans le tableau de bord Cloudant
...
Cloudant s'exécute à une vitesse appropriée afin de transférer les données entre la source et la cible
...
Votre cas d'utilisation peut exiger une réplication plus rapide au détriment des autres services Cloudant.
Il peut
...
contactez l'[équipe de support IBM Cloudant ![External link icon](../images/launch-glyph.svg "External link icon")](mailto:support@cloudant.com){:new_window}.
