...
2.	La redondance et le basculement des données interrégionaux, également connus sous le nom de [Redondance interrégionale pour la reprise après incident](#cross-region-redundancy-for-disaster-recovery).
...
Pour fournir une protection supplémentaire au-delà de la région unique associée à votre compte, vous devez utilisez la [Redondance interrégionale pour la reprise après incident](#cross-region-redundancy-for-disaster-recovery).
...
## Redondance interrégionale pour la reprise après incident 
...
Les principales étapes de la création d'une redondance interrégionale sont les suivantes :
...
3.  Pour les bases de données devant être stockées à l'aide de la redondance interrégionale, configurer des réplications continues bidirectionnelles entre les bases de données correspondantes dans chaque compte.
4.  Concevoir et implémenter vos applications pour que les demandes de données soient acheminées selon que votre environnement a une configuration "active-passive" ou "active-active".
  Pour plus d'informations sur la configuration, voir [Configuration de {{site.data.keyword.cloudant_short_notm}} pour la reprise après incident interrégionale](/docs/services/Cloudant?topic=cloudant-configuring-ibm-cloudant-for-cross-region-disaster-recovery#configuring-ibm-cloudant-for-cross-region-disaster-recovery).
...
En résumé, la redondance interrégionale ressemble à la fonctionnalité de haute disponibilité,
...
## Sauvegarde et reprise de bases de données
...
La [redondance interrégionale pour la reprise après incident](#cross-region-redundancy-for-disaster-recovery) assure aux applications une fonctionnalité de reprise après incident.
...
## Prochaines étapes de votre stratégie de protection des données
...
Pour obtenir des idées et des exemples expliquant les modalités d'intégration de ces outils dans votre stratégie de protection des données, voir le [guide de sauvegarde et de reprise {{site.data.keyword.cloudant_short_notm}}](/docs/services/Cloudant?topic=cloudant-ibm-cloudant-backup-and-recovery#ibm-cloudant-backup-and-recovery).
