...
CouchBackup 工具存在[限制](#limitations)。
{: important}
...
* 在创建备份文件时，通过[“压缩”备份](#compressing-a-backup-file)文件来节省磁盘空间。
* [定期](#hourly-or-daily-backups-that-use-cron-)自动创建数据库的备份。
...
* 在一个任务中备份多个数据库。执行这种备份的方法可以是通过使用 [`_all_dbs`](/docs/services/Cloudant?topic=cloudant-databases#get-a-list-of-all-databases-in-the-account) 调用来确定所有数据库，然后分别执行每个数据库的备份。
...
返回到 [{{site.data.keyword.cloudant_short_notm}} 灾难恢复指南](/docs/services/Cloudant?topic=cloudant-disaster-recovery-and-backup#disaster-recovery-and-backup)，以了解 {{site.data.keyword.cloudant_short_notm}} 为完全灾难恢复设置提供的其他功能。
