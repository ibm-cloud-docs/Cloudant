...
本文更细致地讨论了如何对应用程序数据建模以使其能大规模地高效工作。
{:shortdesc}

_（本指南基于 Mike Rhodes 的博客文章：[对数据建模以进行扩展的五大建议 ![外部链接图标](../images/launch-glyph.svg "外部链接图标")](https://cloudant.com/blog/my-top-5-tips-for-modelling-your-data-to-scale/){:new_window}，最初发布于 2013 年 12 月 17 日。）_

Cloudant 上的数据建模方式将显著影响应用程序能够如何扩展。我们的底层数据模型与关系模型有很大差异，忽略这种差异可能是导致日后发生性能问题的原因。
...
`https://<account>.cloudant.com/` 界面基于的是分布式数据库。在集群内，文档存储在若干分片中，这些分片共同构成了数据库。随后，这些分片在集群中的各个节点之间分布。通过这一机制，我们可以支持大小高达很多太字节的数据库。
...
在关系数据库中，对数据规范化通常是存储数据的最高效方式。在可以使用 JOIN 轻松组合多个表中的数据的情况下，这样做非常适合。在 Cloudant 中，您更有可能需要对每个数据块发出 HTTP GET 请求，所以减少构建已建模实体全貌所需的请求数，能让您更快地向用户显示信息。

通过使用视图，您可以获得对数据规范化的许多优点，同时保留去规范化版本以提高效率。

例如，在关系模式中，您通常会在单独的表中表示标记，并使用连接表将标记与其关联的文档连接在一起，从而允许快速查找带有给定标记的所有文档。

在 Cloudant 中，您会将标记存储在每个文档内的列表中。然后，您会使用视图通过[在视图的 map 函数中将每个标记作为键发出](../api/creating_views.html)来获取带有给定标记的文档。接着，查询给定键的视图将提供带有该标记的所有文档。
...
避免冲突文档有助于加快对 Cloudant 数据库执行的许多操作。这是因为有一个过程会运算出每次读取文档时使用的当前获胜修订版：检索单个文档、使用 `include_docs=true` 进行调用、视图构建，等等。

获胜修订版是文档总体树中的特定修订版。回想一下，Cloudant 上的文档实际上是修订版树。对文档发出请求时，任意但确定性算法将选择此树的其中一个未删除的叶。处理分支因子较高的更大型树所需时间比没有分支或分支极少的文档树更长：需要遍历每个分支，以查看它是否有可能成为获胜修订版。然后，潜在的胜利者需要相互进行比较，以做出最终选择。

Cloudant 显然能够很好地处理少量分支（毕竟，复制得以开展的前提条件是文档可以分支以避免废弃数据），但如果分支数量多得难以想象，尤其是在冲突未解决的情况下，遍历文档树会变得非常耗时，而且会占用大量内存。

## 构建带有冲突的解决方法

在像 Cloudant 这样的最终一致性系统中，冲突最终会发生。如上所述，这是实现可扩展性和数据弹性要付出的代价。
...
上述建议说明了数据建模会影响应用程序性能的一些情况。Cloudant 的数据存储有一些特定的特性，既要小心提防也要加以利用，这样才能确保数据库性能随着应用程序的增长而扩展。我们理解，转换过程可能令人困惑，所以我们随时愿意提供建议。
