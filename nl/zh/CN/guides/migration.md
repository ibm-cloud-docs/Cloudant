...
[{{site.data.keyword.cloudantfull}}](https://www.ibm.com/cloud/cloudant) 的“数据库即服务”产品是在多租户集群上运行的 JSON 文档库。该服务提供了具有可预测成本、可扩展性和服务级别协议 (SLA) 的地理位置供您选择。
...
{{site.data.keyword.cloudant_short_notm}} 共享套餐|旧的现买现付多租户 {{site.data.keyword.cloudant_short_notm}} 服务。`cloudant.com` 共享套餐已于 2018 年 3 月引退。{{site.data.keyword.cloudant_short_notm}} 共享套餐于 2016 年 10 月起就已不推荐用于新注册，并将于 2018 年第四季度引退。
...
度量值|描述
-------|------------
每秒查找次数|执行简单文档访存的速率，例如通过文档的 `_id` 检索文档。
每秒写入次数|数据写入数据库的速率。用于处理文档创建、更新或删除的 API 调用视为“写入”。
每秒查询次数|查询数据库的速率，通常通过访问 `_find` 端点或使用辅助 MapReduce 索引来执行查询。
存储空间|JSON 数据、附件和辅助索引占用的磁盘空间量。

例如，轻量套餐免费提供 20 次查找/秒、10 次写入/秒、5 次查询/秒以及 1 GB 存储空间。如果您“首次尝试”使用产品以及处于产品开发过程中，那么此套餐是理想的选择。应用程序进入生产环境时，必须切换到标准套餐。标准套餐的最小包提供 100 次查找/秒、50 次写入/秒、5 次查询/秒以及 20 GB 存储空间（超过此值的存储空间按 GB 收费），每月约 76.65 美元。 
...
系统将根据在任何给定的每小时窗口中选择的最高容量对您计费。数据库吞吐量可以向上扩展以处理季节性需求，然后在静默期再次向下扩展。您的每月帐单总是可预测的；升级会自动执行；并且您的 SLA 为 [99.95%](http://www-03.ibm.com/software/sla/sladb.nsf/sla/bm-6627-03)。 
如果在给定一秒内超过查找次数、写入次数和查询次数的配额，那么 {{site.data.keyword.cloudant_short_notm}} 集群会使用 `HTTP 429` 进行响应。应用程序可能会稍后重试该请求 - 我们的官方库提供了一个选项，可用于在指数退避的情况下重试此类请求。 

## 我的 {{site.data.keyword.cloudant_short_notm}} 套餐是哪种类型？

如果您使用的是 {{site.data.keyword.cloudant_short_notm}}，那么 {{site.data.keyword.cloud_notm}}“仪表板”会显示您的所有 {{site.data.keyword.cloudant_short_notm}} 实例以及“套餐”列。具体称为轻量套餐、标准套餐和专用硬件套餐。显示共享专用套餐名称的任何 {{site.data.keyword.cloudant_short_notm}} 实例都属于 {{site.data.keyword.cloudant_short_notm}} 专用环境，而不属于公共共享套餐。显示共享套餐名称或在该列中缺少套餐名称的任何 {{site.data.keyword.cloudant_short_notm}} 实例使用的是不推荐的共享套餐。在以下示例中，“Cloudant NoSQL DB-ix”和“Cloudant_NewConsole”实例使用的是不推荐的共享套餐。  

![云仪表板](../images/ibmclouddashboard.png)

您还可以向下钻取到特定实例，并查看“套餐”选项卡。不推荐的共享套餐上的 {{site.data.keyword.cloudant_short_notm}} 实例没有突出显示的套餐。{{site.data.keyword.cloudant_short_notm}} 标准套餐上的实例类似于以下示例： 
...
2.  选择要迁移的 {{site.data.keyword.cloudant_short_notm}} 实例。 
3.  选择左侧导航中的**套餐**选项卡。 
4.  从价格套餐列表中，选中**标准**复选框。
![轻量](../images/migrate3.png)
5.  单击页面底部的**升级**。您的所有现有数据都会保留。
...
登录到 {{site.data.keyword.cloud_notm}} 帐户后，请添加 {{site.data.keyword.cloudant_short_notm}} 服务。单击仪表板上的`创建资源`按钮，然后单击`数据库`和 `Cloudant`。有关更多信息，请参阅[如何在 {{site.data.keyword.cloud_notm}} 上创建 {{site.data.keyword.cloudant_short_notm}} 实例](../tutorials/create_service.html#creating-an-ibm-cloudant-instance-on-ibm-cloud)。 
...
如果要从 {{site.data.keyword.cloudant_short_notm}} 企业服务或 {{site.data.keyword.cloudant_short_notm}} 共享套餐移动，那么必须重新访问应用程序的 {{site.data.keyword.cloudant_short_notm}} 使用情况，以确保它已准备好处理标准套餐的容量限制。有关更多信息，请参阅 [{{site.data.keyword.cloudant_short_notm}} API 的工作方式](../offerings/bluemix.html#provisioned-throughput-capacity)。最重要的是，必须确保在超出所供应的吞吐量容量时，应用程序可以处理 HTTP 响应：`429：请求太多`。 

对于偶然发生的超过套餐容量的流量峰值，重试获得 `429` 响应的请求是可接受的。如果应用程序流量经常生成 `429` 响应，那么您很可能需要升级到更大的套餐。

此外，值得注意的是，{{site.data.keyword.cloudant_short_notm}} 上的最大单个文档大小为 1 MB，如果超出此限制，您将收到 `413：请求实体太大`消息。有关更多信息，请参阅[请求和文档大小限制](../offerings/bluemix.html#request-and-document-size-limits)。 

### 第 4 步：将旧服务中的数据迁移到新服务 

设置从现有服务到 {{site.data.keyword.cloudant_short_notm}} 帐户的连续复制。有关更多信息，请参阅[复制指南](../guides/replication_guide.html#replication)和 [API 文档](../api/replication.html#replication)中关于如何设置和监视复制任务的内容。
...
准备好移至新实例时，可使用为 {{site.data.keyword.cloudant_short_notm}} 实例自动生成的新帐户 URL 和凭证来更新应用程序。有关更多信息，请参阅[供应教程](../tutorials/create_service.html#creating-a-cloudant-instance-on-bluemix)中关于如何获取 {{site.data.keyword.cloudant_short_notm}} 实例的服务凭证的内容。
...
{{site.data.keyword.cloudant_short_notm}} 建议使用 [couchbackup](../guides/backup-cookbook.html#cloudant-nosql-db-backup-and-recovery) 实用程序将数据导出到磁盘。[{{site.data.keyword.cloud_notm}} Object Storage](https://www.ibm.com/cloud/object-storage) 是一种低价的可扩展解决方案，可用于存储导出的文件。 
