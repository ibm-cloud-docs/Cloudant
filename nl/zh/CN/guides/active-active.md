...
> **注**：对于主动/主动部署，必须落实用于管理冲突的策略。因此，在考虑此体系结构之前，请确保了解[复制](../api/replication.html)和[冲突](mvcc.html#distributed-databases-and-conflicts)的工作方式。
...
[创建数据库](../api/database.html#create)，即要在各帐户之间复制的数据库。
...
最好将 [API 密钥](../api/authorization.html#api-keys)用于持续复制。这样做的优点在于，如果主帐户详细信息更改（例如，在密码重置后），复制可以继续保持不变。
...
> **注**：请仔细记下密码。以后无法再找回此密码。

### 第 3 步：授予访问许可权

[授予 API 密钥许可权](../api/authorization.html#modifying-permissions)以对两个数据库执行读写操作。

如果还要复制索引，请分配管理许可权。

使用 {{site.data.keyword.cloudant_short_notm}}“仪表板”，或者查看[授权](../api/authorization.html)信息，以了解有关如何通过编程方式授予许可权的详细信息。
...
> **注：**如果此步骤因 `_replicator` 数据库不存在而失败，请创建该数据库。
...
确定是否需要故障转移的简单测试是将主数据库端点用作“脉动信号”。例如，发送到主数据库端点的简单 `GET` 请求通常会返回[有关数据库的详细信息](../api/database.html#getting-database-details)。如果未接收到响应，那么可能指示有必要进行故障转移。
...
> **注：**如果是根据运行状况测试来实施请求重新路由或故障转移，那么您可能希望合并对应的检查，以避免过早重新路由回仍在恢复的服务实例。
...
提供了有关[监视复制状态](../api/advanced_replication.html#replication-status)的更多信息。

> **注：**如果正在持续更改数据库，那么复制状态不可能为 0。您必须决定哪个状态阈值是可接受的，或哪个表示错误状态。

### 索引

* 索引是否足够新？使用[活动任务](../api/active_tasks.html)端点可检查此项。
