...
{{site.data.keyword.cloudant_short_notm}} 會在執行服務以及最佳化使用者體驗的過程中，處理有限的客戶 PI。{{site.data.keyword.cloudant_short_notm}} 會適當地使用電子郵件來與客戶聯絡。監視客戶與 {{site.data.keyword.cloudant_short_notm}} 儀表板的互動是 {{site.data.keyword.cloudant_short_notm}} 處理 PI 的另一種方式。
