...
此時，已將一小組資料儲存為資料庫內的文件。您可以從資料庫中執行該資料的最小或完整擷取。最小擷取會取得_關於_ 文件的基本資料。完整擷取也會包括文件_內_ 的資料。
...
* 若要執行最小擷取，請執行下列動作：
  ...
  2. 顯示陣列中元素的內容。
    ...
    > **附註：**`u'` 字首僅是指示 Python 正在顯示 Unicode 字串。 
    
    如果稍微整理外觀，可以看到獲得的最小文件明細如下所示：
  ...
  > **附註：**NoSQL 資料庫（例如 {{site.data.keyword.cloudant_short_notm}}）的本質意味無需套用簡單概念，例如儲存在資料庫的第一份文件一律會是結果清單中傳回的第一個。
...
您也可以呼叫 Cloudant [`/_all_docs` 端點](api/database.html#get-documents)，要求一份包含所有文件的清單及其內容。
