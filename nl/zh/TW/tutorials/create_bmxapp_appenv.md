...
即會出現 {{site.data.keyword.cloud_notm}} 上可用的 Cloud Foundry 應用程式清單。

4.  按一下 '`Python`' 項目：<br/>
    ![{{site.data.keyword.cloud_notm}} Python Cloud Foundry 應用程式](images/img0013.png)<br/>
...
5.  使用`建立 Cloud Foundry 應用程式`表單，以指定及建立 Python Cloud Foundry 應用程式的環境。請輸入應用程式的名稱，例如 '`Cloudant Python`'。系統會自動為您產生主機名稱，但您可以進行自訂：<br/>
    ![{{site.data.keyword.cloud_notm}} Python Cloud Foundry 應用程式名稱](images/img0014.png)<br/>
    >   **附註**：在 {{site.data.keyword.cloud_notm}} 網域內，主機名稱必須是唯一的。在此範例中，網域為 '`mybluemix.net`'，並提供完整主機名稱 '`Cloudant-Python.mybluemix.net`'。

6.  按一下「`建立`」來建立應用程式環境：<br/>
    ![建立 {{site.data.keyword.cloud_notm}} Python Cloud Foundry 應用程式](images/img0015.png)

7.  在短暫暫停之後，會出現新應用程式環境的「`開始使用`」視窗。會在環境內自動建立測試應用程式。將自動啟動應用程式，如綠色圖示及`您的應用程式正在執行中`狀態所示。應用程式是一種「活動訊號」程式，足以顯示可供您使用的新應用程式環境。按一下`儀表板`鏈結，以回到 {{site.data.keyword.cloud_notm}} 帳戶儀表板。<br/>
...
您現在具備 {{site.data.keyword.cloud_notm}} Python 應用程式環境，可供使用。
...
指導教學的這一節說明如何使用應用程式的配置及管理區域來連接 {{site.data.keyword.cloud_notm}} 應用程式環境與服務。

1.  從 {{site.data.keyword.cloud_notm}} 儀表板中，按一下應用程式的項目。<br/>
...
{{site.data.keyword.cloud_notm}} 工具箱提供額外功能，而這些是使用在 {{site.data.keyword.cloud_notm}} 環境內管理及執行之應用程式的必要功能。

> **附註**：請確定您同時安裝 Cloud Foundry _及_ {{site.data.keyword.cloud_notm}} 工具箱。
...
4.  <div id='checkCFversion'></div>若要確認您有運作中的 Cloud Foundry 工具箱，請在提示中執行下列指令：
    ...
    >   **附註**：Cloud Foundry 工具箱版本必須是 6.11 或更新版本，才能與 {{site.data.keyword.cloud_notm}} 相容。

### 安裝 {{site.data.keyword.cloud_notm}} 工具箱

請使用下列步驟，在系統上下載及安裝 {{site.data.keyword.cloud_notm}} 工具箱。

1.  應用程式的`開始使用`視窗提供了 {{site.data.keyword.cloud_notm}} 工具箱的下載鏈結：<br/>
    ![下載 Cloud Foundry 工具箱的鏈結](images/img0027.png)

2.  按一下此鏈結會將您帶到[下載頁面 ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](https://console.bluemix.net/docs/cli/reference/bluemix_cli/get_started.html#getting-started){:new_window}：<br/>
    ![{{site.data.keyword.cloud_notm}} 工具箱下載頁面](images/img0028.png)
...
4.  若要確認您有運作中的 {{site.data.keyword.cloud_notm}} 工具箱，請在提示中執行下列指令：
...
現在提供使用 {{site.data.keyword.cloud_notm}} 應用程式的工具。下一步是取得「入門範本」資料，協助您建立 {{site.data.keyword.cloud_notm}} 應用程式。

<div id="starter"></div>

## 「入門範本」應用程式

指導教學的這一節說明 {{site.data.keyword.cloud_notm}} 入門範本應用程式，並解釋如何進行自訂來存取 {{site.data.keyword.cloudant_short_notm}} 資料庫實例。

{{site.data.keyword.cloud_notm}} 入門範本應用程式是建立可運作 {{site.data.keyword.cloud_notm}} 應用程式所需原始檔及配置檔的最小可能集合。在某些方面，它類似 ['Hello World' 應用程式 ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](https://en.wikipedia.org/wiki/%22Hello,_World!%22_program){:new_window}；它只足以顯示基本系統及配置正確地運作。

{{site.data.keyword.cloud_notm}} 入門範本應用程式是您開發 {{site.data.keyword.cloud_notm}} 應用程式時必須修改或擴充的範例檔案的保存檔。
...
{{site.data.keyword.cloud_notm}} Python 入門範本應用程式的 '`Procfile`' 類似下列範例：
