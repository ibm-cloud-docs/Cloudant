...
# 建立簡單的 Bluemix 應用程式以存取 Cloudant 資料庫：應用程式環境
...
1.  登入 {{site.data.keyword.Bluemix_notm}} 帳戶。您可以在 [http://bluemix.net ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](http://bluemix.net){:new_window} 找到 {{site.data.keyword.Bluemix_notm}} 儀表板。在您利用使用者名稱及密碼進行鑑別之後，會出現 {{site.data.keyword.Bluemix_notm}} 儀表板：<br/>
...
5.  使用「`建立 Cloud Foundry 應用程式`」表單，以指定及建立 Python Cloud Foundry 應用程式的環境。請輸入應用程式的名稱，例如 '`Cloudant Python`'。系統會自動為您產生主機名稱，但您可以進行自訂：<br/>
    ...
    >   **附註**：在 {{site.data.keyword.Bluemix_notm}} 網域內，主機名稱必須是唯一的。在此範例中，網域為 '`mybluemix.net`'，並提供完整主機名稱 '`Cloudant-Python.mybluemix.net`'。

6.  按一下「`建立`」來建立應用程式環境：<br/>
    ![建立 {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 應用程式](images/img0015.png)

7.  在短暫暫停之後，會出現新應用程式環境的「`開始使用`」視窗。會在環境內自動建立測試應用程式。將自動啟動應用程式，如綠色圖示及`您的應用程式正在執行中`狀態所示。應用程式是一種「活動訊號」程式，足以顯示可供您使用的新應用程式環境。按一下`儀表板`鏈結，以回到 {{site.data.keyword.Bluemix_notm}} 帳戶儀表板。<br/>
    ![第一次執行新的 {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 應用程式](images/img0016.png)

8.  您的儀表板現在包括新建立的應用程式環境：<br/>
    ![出現在儀表板中的基本 {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 應用程式](images/img0017.png)

您現在具備可開始使用的 {{site.data.keyword.Bluemix_notm}} Python 應用程式環境。
...
3.  本指導教學的[必要條件](create_bmxapp_prereq.html#prerequisites)是現有 {{site.data.keyword.cloudant_short_notm}} 資料庫實例。按一下「`連接現有項目`」，以建立該資料庫實例與應用程式之間的連線：<br/>
...
## Cloud Foundry 及 Bluemix 指令工具箱

指導教學的這一節說明必須安裝才能使用 {{site.data.keyword.Bluemix_notm}} 環境、應用程式及服務的工具箱。

[Cloud Foundry ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](https://en.wikipedia.org/wiki/Cloud_Foundry){:new_window} 工具箱是工具集合，可用來使用 Cloud Foundry 相容環境中部署的應用程式。請將這些工具用於作業（例如更新已部署的應用程式），或者啟動及停止執行中應用程式。
...
1.  應用程式的「`開始使用`」視窗提供 {{site.data.keyword.Bluemix_notm}} 工具箱的下載鏈結：<br/>
...
指導教學的這一節說明 {{site.data.keyword.Bluemix_notm}} 入門範本應用程式，以及說明如何進行自訂來存取 {{site.data.keyword.cloudant_short_notm}} 資料庫實例。

{{site.data.keyword.Bluemix_notm}} 入門範本應用程式是建立可運作 {{site.data.keyword.Bluemix_notm}} 應用程式所需來源及配置檔的最小可能集合。在某些方面，它類似 ['Hello World' 應用程式 ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](https://en.wikipedia.org/wiki/%22Hello,_World!%22_program){:new_window}；只足以顯示基本系統及配置正確地運作。
...
更具體而言，'`Procfile`' 是一種 Cloud Foundry 構件，定義應用程式處理程序類型，以及執行應用程式的指令。[這裡 ![外部鏈結圖示](../images/launch-glyph.svg "外部鏈結圖示")](https://docs.cloudfoundry.org/buildpacks/prod-server.html#procfile){:new_window} 提供 '`Procfile`' 的相關資訊。
...
入門範本應用程式保存檔中包括入門範本 '`server.py`' Python 原始檔。'`server.py`' 檔案是針對應用程式進行修改。或者，建立全新的 Python 原始檔。然後，更新 '`Procfile`'，以在啟動應用程式時使用新檔案。
...
您一般不需要修改 '`manifest.yml`' 檔案，不過，最好瞭解應用程式為什麼必須要有它才能運作。
