...
抄寫處理程序會在內部將其狀態寫入來源及目的地資料庫中所儲存的「檢查點」文件內。檢查點容許從其停止位置繼續抄寫作業，而不需要從頭開始。在您要求抄寫時提供 [`"use_checkpoints": false`](../api/replication.html#checkpoints) 選項，即可防止建立檢查點。如果您的抄寫是要有效率地從其上一個已知位置繼續，則這有助於開啟這項功能。

## 許可權

需要有管理者存取權，才能將文件插入 `_replicator` 資料庫。來源及目標參數中所提供的登入認證不需要完整的管理者權限。如果認證可以進行下列作業，則這就已足夠：
...
若要結合已知位置中的 changes 資訊來源，請傳遞含有您要從其開始的序號的 [`since` 引數](../api/database.html#the-since-argument)。

_使用 HTTP 提供 `since` 選項以結合已知位置的 `_changes` 資訊來源的範例（簡短）：
...
_使用指令行提供 `since` 選項以結合已知位置的 `_changes` 資訊來源的範例（簡短）：
