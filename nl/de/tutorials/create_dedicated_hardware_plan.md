...
3.  Wählen Sie den Plan für `dedizierte Hardware` in der Liste der Preisstrukturpläne aus:<br/>
    ![Preisstrukturpläne](images/pricing_plan.png)
    
4.  Geben Sie die folgenden Parameter ein: <br/>
    -   Geben Sie einen Servicenamen ein.<br/>
    -   Geben Sie die Region und den Standort an, in der bzw. an dem die Bereitstellung stattfinden soll. Bei der Region/dem Standort handelt es sich um eine sechs {{site.data.keyword.cloud_notm}}-Hauptregionen, wo die Instanz bereitgestellt werden soll. Der tatsächliche physische Standort der Instanz wird durch den in der folgenden Liste beschriebenen Standortparameter bestimmt.<br/>
    -   Wählen Sie eine Ressourcengruppe aus.</br>
    -   Fügen Sie einen Tag hinzu. 
    -   Wählen Sie einen Standort für die Bereitstellung aus. Dabei handelt es sich um den physischen Standort der Instanz, der sich an einem beliebigen {{site.data.keyword.cloud_notm}}-Standort befinden kann, einschließlich der wichtigsten Regionen und Standorte außerhalb der Hauptregionen. Weitere Informationen finden Sie in [{{site.data.keyword.IBM}} Global Data Centers ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://www.ibm.com/cloud/data-centers/){:new_window}.<br/>
    ...
    ![Dedizierte Hardware konfigurieren](images/select_deployment_location.png)
...
5.  Klicken Sie auf die Schaltfläche `Erstellen`, um den Bereitstellungsprozess zu starten. <br/>
    ...
    ![Dedizierte Hardware bereitstellen](images/create_button_provision.png)
    ...
    ![Alternative Version der dedizierten Hardware bereitstellen](images/create_button_provision2.png)
...
3.  Klicken Sie in den Preisstrukturplänen auf `Standard`. <br/>
    ![Standardpreisstrukturplan](images/standard_pricing_plan.png)
...
4.  Geben Sie die folgenden Parameter ein: <br/>
    ...
    -   Wählen Sie eine Umgebung aus, in der die Standardplaninstanz bereitgestellt werden soll. Alle verfügbaren Instanzen von Umgebungen mit dedizierter Hardware, die im Konto bereitgestellt sind, werden in der Dropdown-Liste angezeigt.</br>
    ![Standardinstanz konfigurieren](images/select_environment.png)
...
5.  Klicken Sie auf die Schaltfläche `Erstellen`.<br/>
    ...
    ![Standardinstanz bereitstellen](images/create_button_provision_standard.png)
...
Weitere Informationen zu den Feldern, die in den Serviceberechtigungsnachweisen enthalten sind, finden Sie im [IAM-Leitfaden](/docs/services/Cloudant?topic=cloudant-ibm-cloud-identity-and-access-management-iam-#ibm-cloud-identity-and-access-management-iam-).
...
`service-endpoints` | Ein optionaler Parameter zum Füllen des URL-Felds in den Serviceberechtigungsnachweisen mit einem internen Endpunkt für die Verbindung mit dem Service über das interne {{site.data.keyword.cloud_notm}}-Netz. Geben Sie diesen Parameter nicht an, um die URL mit einem externen Endpunkt zu füllen, der allgemein zugänglich ist. Gilt nur für Instanzen des Standard-Plans, die in Umgebungen mit dedizierter Hardware bereitgestellt wurden, die interne Endpunkte unterstützen. Der Befehl verursacht einen Fehler des Typs 400, wenn die Umgebung keine internen Endpunkte unterstützt. 
