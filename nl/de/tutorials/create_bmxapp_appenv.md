...
# Einfache Bluemix-Anwendung für den Zugriff auf eine Cloudant-Datenbank erstellen: Anwendungsumgebung

In diesem Abschnitt des Lernprogramms wird beschrieben, wie Sie die Anwendungsumgebung für das Erstellen einer {{site.data.keyword.Bluemix}}-Anwendung einrichten.
{:shortdesc}

<div id="creating"></div>

## Bluemix-Anwendungsumgebung erstellen

1.  Melden Sie sich bei Ihrem {{site.data.keyword.Bluemix_notm}}-Konto an. Das {{site.data.keyword.Bluemix_notm}}-Dashboard finden Sie hier:
    ...
    wird das {{site.data.keyword.Bluemix_notm}}-Dashboard angezeigt: <br/>
...
3.  Klicken Sie auf den Eintrag `Cloud Foundry-Apps` unter der Überschrift `Apps`: <br/>
    ...
    Eine Liste der Cloud Foundry-Anwendungen, die unter {{site.data.keyword.Bluemix_notm}} verfügbar sind, wird angezeigt. 
...
5.  Verwenden Sie das Formular zum `Erstellen einer Cloud Foundry-App`, um die Umgebung
    ...
    ![{{site.data.keyword.Bluemix_notm}} Python Cloud Foundry-App-Namen](images/img0014.png)<br/>
    >   **Hinweis**: Der Hostname muss in der {{site.data.keyword.Bluemix_notm}}-Domäne eindeutig sein. In diesem Beispiel heißt die Domäne `mybluemix.net`, was den vollständigen Hostnamen `Cloudant-Python.mybluemix.net` ergibt. 

6.  Klicken Sie auf `Erstellen`, um die Anwendungsumgebung zu erstellen: <br/>
    ![{{site.data.keyword.Bluemix_notm}} Python Cloud Foundry-App erstellen](images/img0015.png)
...
7.  Nach einer kurzen Pause wird das Fenster
    ...
    Klicken Sie auf den Link `Dashboard`, um zu Ihrem {{site.data.keyword.Bluemix_notm}}-Konto-Dashboard zurückzukehren. <br/>
    ![Die neue {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry-App wird zum ersten Mal ausgeführt](images/img0016.png)

8.  Ihr Dashboard enthält jetzt die neu erstellte Anwendungsumgebung: <br/>
    ![Grundlegende {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry-App im Dashboard](images/img0017.png)

Sie haben jetzt eine einsatzbereite {{site.data.keyword.Bluemix_notm}} Python-Anwendungsumgebung. 

Um mit einer {{site.data.keyword.cloudant_short_notm}}-Datenbankinstanz zu arbeiten, ist der nächste
Schritt, eine Verbindung zwischen der Anwendungsumgebung und der Datenbankinstanz herzustellen. 

<div id="connecting"></div>

## Bluemix-Anwendungen und -Services verbinden

In diesem Abschnitt des Lernprogramms wird erläutert, wie Sie
{{site.data.keyword.Bluemix_notm}}-Anwendungsumgebungen und -Services mithilfe des
Konfigurations- und Verwaltungsbereichs Ihrer Anwendung verbinden. 

1.  Klicken Sie in Ihrem {{site.data.keyword.Bluemix_notm}}-Dashboard auf den Eintrag für Ihre Anwendung. <br/>
    ![Auswahl Ihrer Anwendung im {{site.data.keyword.Bluemix_notm}}-Dashboard](images/img0018.png)<br/>
...
2.  Klicken Sie auf den Link `Verbindungen`, um die Anwendungsumgebung mit einem anderen Service zu verbinden: <br/>
    ![Auswahl der Verbindungskonfiguration für Ihre {{site.data.keyword.Bluemix_notm}}-Anwendung](images/img0019.png)<br/>
...
Der nächste Schritt besteht darin, sicherzustellen, dass die erforderlichen Tools installiert sind, um mit {{site.data.keyword.Bluemix_notm}}-Anwendungen zu arbeiten. 

<div id="toolkits"></div>

## Die Cloud Foundry- und Bluemix-Befehlszeilentoolkits

In diesem Abschnitt des Lernprogramms werden die Toolkits beschrieben, die installiert werden müssen, um mit Ihrer {{site.data.keyword.Bluemix_notm}}-Umgebung, Ihren Anwendungen und Services zu arbeiten. 
...
Das {{site.data.keyword.Bluemix_notm}}-Toolkit stellt zusätzliche Funktionen bereit, die für die Arbeit mit Anwendungen erforderlich sind,
die in einer {{site.data.keyword.Bluemix_notm}}-Umgebung gehostet und ausgeführt werden. 
...
[hier ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://console.ng.bluemix.net/docs/cli/index.html){:new_window} verfügbar.
...
Falls die unterstützte Version 6.11 oder höher ist, ist sie kompatibel mit {{site.data.keyword.Bluemix_notm}} und kann verwendet werden.
...
4.  <div id='checkCFversion'></div>Prüfen Sie, ob Sie ein funktionierendes Cloud Foundry-Toolkit haben,
    ...
    >   **Hinweis**: Für die Kompatibilität mit {{site.data.keyword.Bluemix_notm}} muss die Version des Cloud Foundry-Toolkits 6.11 oder höher sein. 

### Installation des Bluemix-Toolkits

Führen Sie die folgenden Schritte aus, um das {{site.data.keyword.Bluemix_notm}}-Toolkit auf Ihrem System herunterzuladen und zu installieren. 

1.  Ein Link zum Herunterladen des {{site.data.keyword.Bluemix_notm}}-Toolkits ist im Fenster
    `Einführung` Ihrer Anwendung verfügbar: <br/>
    ![Link zum Herunterladen des Cloud Foundry-Toolkits](images/img0027.png)

2.  Wenn Sie auf den Link klicken, werden Sie zu einer
    [Downloadseite ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](http://clis.ng.bluemix.net/ui/home.html){:new_window} weitergeleitet: <br/>
    ![Downloadseite für das Bluemix-Toolkit](images/img0028.png)
...
3.  Laden Sie das entsprechende Installationsprogramm für Ihr System herunter und führen Sie es aus.
    ...
    {{site.data.keyword.Bluemix_notm}}-Toolkit auf Ihrem System installiert. 

4.  Prüfen Sie, ob Sie ein funktionierendes {{site.data.keyword.Bluemix_notm}}-Toolkit haben,
    indem Sie den folgenden Befehl in einer Eingabeaufforderung ausführen: 
...
Die Tools, die mit {{site.data.keyword.Bluemix_notm}}-Anwendungen arbeiten sollen, sind jetzt verfügbar.
Der nächste Schritt besteht darin, die Startermaterialien abzurufen, um eine
{{site.data.keyword.Bluemix_notm}}-Anwendung zu erstellen. 

<div id="starter"></div>

## Starteranwendung

In diesem Abschnitt des Lernprogramms wird eine {{site.data.keyword.Bluemix_notm}}-Starteranwendung beschrieben und
erläutert, wie Sie diese für den Zugriff auf eine {{site.data.keyword.cloudant_short_notm}}-Datenbankinstanz anpassen. 

Eine {{site.data.keyword.Bluemix_notm}}-Starteranwendung ist die kleinstmögliche Sammlung von Quellen-
und Konfigurationsdateien, die erforderlich ist, um eine funktionstüchtige {{site.data.keyword.Bluemix_notm}}-Anwendung zu erstellen.
...
Eine {{site.data.keyword.Bluemix_notm}}-Starteranwendung ist ein Archiv von Beispieldateien, das Sie
im Rahmen der Entwicklung Ihrer {{site.data.keyword.Bluemix_notm}}-Anwendung ändern oder erweitern müssen. 
...
{{site.data.keyword.Bluemix_notm}} benötigt, um Ihre Anwendung auszuführen. 
...
Die `Procfile` für eine {{site.data.keyword.Bluemix_notm}} Python-Starteranwendung
sieht wie folgt aus: 
...
Die Datei für eine {{site.data.keyword.Bluemix_notm}} Python-Starteranwendung sieht wie folgt aus: 
...
-   Die Werte `domain`,
    ...
    als Ihre {{site.data.keyword.Bluemix_notm}}-Anwendung [erstellt](#creating) wurde. 
...
In diesem Lernprogramm greift die Python-Anwendung aber auf eine
{{site.data.keyword.cloudant_short_notm}}-Datenbankinstanz zu.
