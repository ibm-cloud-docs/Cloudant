...
stellen Cloudant-Datenbanken sicher, dass alle Knoten in einem Datenbankcluster nur die [aktuelle Version](../api/document.html) eines Dokuments enthalten.
{:shortdesc}

Da Cloudant-Datenbanken [sukzessive konsistent](cap_theorem.html) sind,
ist dies notwendig, um Inkonsistenzen zwischen Knoten infolge einer Synchronisierung veralteter Dokumente zu vermeiden. 

Multiversion Concurrency Control (MVCC) ermöglicht gleichzeitigen Lese- und Schreibzugriff auf eine Cloudant-Datenbank.
...
Sie beschleunigt sowohl das Lesen als auch das Schreiben von Operationen in Cloudant-Datenbanken, weil es keinen Grund für Datenbanksperren gibt.
MVCC ermöglicht auch eine Synchronisierung zwischen Cloudant-Datenbankknoten. 

## Revisionen

Jedes Dokument in einer Cloudant Datenbank hat ein Feld `_rev`, in dem die zugehörige Revisionsnummer angegeben ist. 
...
Verteilte Datenbanken arbeiten ohne konstante Verbindung zur Hauptdatenbank unter Cloudant,
...
An diesem Punkt sind die Konflikte, die sich auf das Dokument auswirken, gelöst. Sie können dies überprüfen, indem Sie das Dokument erneut mit `GET` abrufen, wobei der Parameter
