...
Daten können aus einer Datenbank in eine andere Datenbank im selben Cloudant-Konto, über Konten und Rechenzentren hinweg, kopiert werden. {:shortdesc}

Daten können sogar zwischen einem Cloudant-Konto und einem mobilen Gerät repliziert werden - mithilfe von
[Cloudant Sync ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://cloudant.com/product/cloudant-features/sync/){:new_window}
...
Das Replikationsprotokoll von Cloudant ist mit einer Reihe von anderen Datenbanken und Bibliotheken kompatibel,
was es zu einer hervorragenden Ergänzung des Internet of Things (IoT) und mobiler Anwendungen macht. 

Dieser Leitfaden führt Sie in die Replikationsfunktionen von Cloudant ein,
bespricht gängige Anwendungsfälle und zeigt, wie Sie Ihre Anwendung einrichten können, damit sie erfolgreich repliziert. 

## Was ist Replikation?

Cloudant ist ein verteilter JSON-Datenspeicher mit einer HTTP-API.
Cloudant kann als Service in mehreren Clouds oder in Ihrem Server-Rack ausgeführt werden.
Dokumente werden in Datenbanken gespeichert und können jede Größe annehmen, da Cloudant die zugehörigen Daten über viele Knoten verteilt.
Replikation ist das Kopieren von Daten aus einer Quellendatenbank in eine Zieldatenbank.
Die Quellen- und Zieldatenbanken müssen sich nicht im selben Cloudant-Konto und auch nicht im selben Rechenzentrum befinden. 
...
Das Cloudant-Dashboard stellt eine komfortable Benutzerschnittstelle zum Starten der Replikation bereit.
Öffnen Sie die Registerkarte 'Replikation' in Ihrem Cloudant-Dashboard und klicken Sie auf die Aktionsschaltfläche `Neue Replikation`.
...
## Replikation über verschiedene Cloudant-Kontos ausführen

Die Quelle und das Ziel einer Replikation sind URLs von Cloudant-Datenbanken,
...
In manchen Fällen kann es unmöglich sein, die Replikation überhaupt auszuführen,
z. B. wenn ein Konto sich hinter der Firewall befindet.
...
## Replikation mithilfe der Cloudant-API starten

Jedes Cloudant-Konto hat eine spezielle Datenbank namens `_replicator`,
...
Feld           | Zweck
----------------|--------
`_id`           | Angabe des Felds `_id` ist optional, kann aber hilfreich sein, um Replikationstasks zu identifizieren. Cloudant generiert einen Wert für Sie, falls Sie keinen angeben. 
`source`        | URL der Cloudant-Quellendatenbank, einschließlich Anmeldeberechtigungsnachweisen. 
`target`        | URL der Cloudant-Zieldatenbank, einschließlich Anmeldeberechtigungsnachweisen. 
...
Allerdings führt die verteilte Struktur von Cloudant-Datenbanken dazu, dass die vom Feed `_changes` bereitgestellte Antwort
...
Cloudant ein sukzessive konsistentes Modell einsetzt.
...
Cloudant mithilfe des [Prüfpunkt](#checkpoints)mechanismus angegeben,
...
Die Anmeldeberechtigungen, die in den Quellen- und Zielparametern angegeben sind, benötigen keine umfassenden Admin-Rechte. Es reicht aus, wenn die Berechtigungsnachweise Folgendes dürfen: 

-   Dokumente im Ziel schreiben.
-   Prüfpunktdokumente in der Quelle und im Ziel schreiben. 

Cloudant verfügt über spezielle `_replicator`-Benutzerberechtigungen.
...
API-Schlüssel können im Cloudant-Dashboard pro Datenbank erstellt und konfiguriert werden. 

![Replikation](../images/replication_guide_5.png)

Sie können auch [programmgesteuert](../api/authorization.html#creating-api-keys) mithilfe der Cloudant-API erstellt werden. 
...
Eine fortlaufende Replikation wird ausgelöst, indem Sie das Kontrollkästchen
`Diese Replikation fortlaufend machen` aktivieren, wenn Sie eine Replikationstask im Cloudant-Dashboard definieren,
oder indem Sie in der Cloudant-API das Flag [`fortlaufend`](../api/replication.html#checkpoints) festlegen. 
...
Sie können den Status der Cloudant-Datenbank `_replicator` jederzeit mithilfe des Dashboards oder der API prüfen. 

Wenn die Replikation fehlschlägt,
z. B. falls die Authentifizierungsnachweise ungültig sind, wird der Fehlerstatus im Dokument `_replicator` aufgezeichnet.
Darüber hinaus kann der Endpunkt `/_active_tasks` des Cloudant-Kontos verwendet werden, um den Fortschritt der Replikation zu sehen.
...
Die Replikation dient nicht zur Datenübertragung zwischen Cloudant-Instanzen.
Das Replikationsprotokoll von Cloudant ist kompatibel mit anderen Datenbanken und Bibliotheken für verschiedene in der Praxis eingesetzte Anwendungen. 
...
die mit Cloudant kommunizieren kann und nur eine minimale Konfiguration erfordert.
Anwendungen sind: 

-   Sicherung: Replizieren Sie Ihre Daten aus Cloudant in Ihre eigenen CouchDB-Datenbanken
...
-   Zuerst lokale Datenerfassung: Schreiben Sie Ihre Daten zunächst in die lokale Apache CouchDB-Instanz,
    replizieren Sie sie dann für eine Langzeitspeicherung, -aggregation und -analyse nach Cloudant. 

### PouchDB

[PouchDB ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](http://pouchdb.com/){:new_window} ist eine Open-Source-,
In-Browser-Datenbank, die ermöglicht, dass Daten bidirektional zwischen dem Browser und Cloudant repliziert werden.
...
PouchDB kann alle geänderten Daten mit Cloudant synchronisieren, wenn eine Internetverbindung aktiv ist.
...
[CloudantSync ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://cloudant.com/cloudant-sync-resources/){:new_window} ist eine Bibliothekengruppe
für iOS und Android, mit der Daten lokal auf mobilen Geräten gespeichert und mit Cloudant synchronisiert werden können, wenn das mobile Gerät verbunden ist.
...
CloudantSync wird häufig in mobilen Anwendungen wie
iPhone- und Android-Spielen verwendet, wobei der Status der Anwendung in Cloudant dauerhaft eingetragen wird,
...
-   Unterteilen von Daten in kleinere Blöcke,
    z. B. Speichern von Daten aus Großbritannien in einer Datenbank und von Daten aus USA in einer anderen. 

<div id="replication-filter-function"></div>

### Replikationsfilterfunktionen

Die gefilterte Replikation in Cloudant ermöglicht die Definition einer JavaScript-Funktion, die den Rückgabewert verwendet,
um zu bestimmen, ob die einzelnen Dokumente in einer Datenbank gefiltert werden sollen oder nicht.
...
Cloudant veröffentlicht die Hinzufügungen,
Bearbeitungen und Löschungen, die sich auf die
Datenbank auswirken, über einen einzelnen HTTP-Feed vom Endpunkt [`_changes`](../api/database.html#get-changes).
...
Verwenden Sie z. B. die [Cloudant-Bibliothek 'Node.js'](../libraries/supported.html#node-js),
...
>   **Hinweis**: Die Reihenfolge von Dokumenten im Feed `_changes` ist nicht immer gleich. 
    Mit anderen Worten: Änderungen werden nicht unbedingt in streng zeitlicher Reihenfolge angezeigt. Grund ist, dass Daten von mehreren Cloudant-Knoten zurückgegeben werden und möglicherweise Konsistenzregeln gelten.
...
API-Schlüssel werden im Cloudant-Dashboard oder [über die API](../api/authorization.html#creating-api-keys) generiert.
Jedem Schlüssel können individuelle Berechtigungen zugewiesen werden, die sich auf eine bestimmte Cloudant-Datenbank beziehen.
Cloudant muss die zugehörigen Prüfpunktdokumente am Ende des Lesevorgangs der Replikation schreiben können, sonst wird kein Status gespeichert und die Replikation kann nicht ab dem Punkt fortgesetzt werden, an dem Sie gestoppt wurde.
...
Wenn der Wert größer als 1 GB ist,
wenden Sie sich an das [IBM Cloudant-Support-Team ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](mailto:support@cloudant.com){:new_window}. 
...
Es wird leicht vergessen, die Sie bereits die Replikation zwischen zwei Datenbanken eingerichtet haben, und so aus Versehen zusätzliche Replikationsprozesse erstellen. Jeder Replikationsjob ist unabhängig von den anderen,
deshalb hindert Cloudant Sie nicht daran, zusätzliche Replikationsprozesse zu erstellen.
Aber jede Replikationstask verbraucht Systemressourcen. 

Sie können Ihre aktiven Replikationen im Cloudant-Dashboard prüfen,
um sicherzustellen, dass keine unerwünschten Replikationstasks ausgeführt werden.
Löschen Sie alle `_replicator`-Dokumente, die nicht mehr benötigt werden. 

## Replikationsgeschwindigkeit optimieren

Standardmäßig werden Cloudant-Replikationen mit einer angemessenen Geschwindigkeit ausgeführt, um die Daten von der Quelle ins Ziel zu übertragen, ohne dass sich dies negativ auf die Leistung auswirkt.
Die Entscheidung zwischen Replikationsrate und Clusterleistung für andere Tasks ist immer ein Kompromiss.
Ihr Anwendungsfall macht vielleicht eine schnellere Replikation auf Kosten anderer Cloud-Services erforderlich.
