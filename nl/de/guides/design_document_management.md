...
Der skalierbare JSON-Datenspeicher von Cloudant verfügt über verschiedene Abfragemechanismen,
...
Die [Suchindizes](../api/search.html) und [MapReduce-Ansichten](../api/creating_views.html)
von Cloudant werden durch Hinzufügen von Entwurfsdokumenten zu einer Datenbank konfiguriert.
...
Einfaches Beispiel: Angenommen, wir haben eine einfache Sammlung von Datendokumenten,
...
Jedes Datendokument enthält einen Namen, einen Hauptteil und eine Zeitmarke. Wir möchten eine [MapReduce-Ansicht](../api/creating_views.html) erstellen, um unsere Dokumente nach Zeitmarke zu sortieren. 
...
Sobald das Entwurfsdokument gespeichert wurde, löst Cloudant serverseitige Prozesse aus, um die Ansicht `fetch/by_ts` zu erstellen.
...
Die Funktion gibt das ausgegebene Schlüssel/Wert-Paar zurück. Im Verlauf der Iteration wird jedes Schlüssel/Wert-Paar in einem B-Tree-Index gespeichert. Nach dem erstmaligen Erstellen des Index
...
-   Die Erstellung eines Index vollzieht sich asynchron.
    Cloudant bestätigt, dass unser Entwurfsdokument gesichert wurde,
    aber um den Fortschritt bei der Erstellung des Index zu prüfen,
    müssen wir den Cloudant-Endpunkt [`_active_tasks`](../api/active_tasks.html) abfragen. 
...
Wenn dieses Entwurfsdokument gespeichert wird,
macht Cloudant den alten Index vollständig ungültig und startet die Erstellung eines völlig neuen Index,
wobei jedes einzelne Dokument iteriert wird.
...
Eine andere Methode, die [hier ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](http://wiki.apache.org/couchdb/How_to_deploy_view_changes_in_a_live_environment){:new_window} beschrieben ist,
verlässt sich darauf, dass Cloudant erkennt, wenn zwei identische Entwurfsdokumente vorhanden sind, und keine Zeit und Ressourcen dafür verschwendet, bereits vorhandene Ansichten ein zweites Mal zu erstellen.
...
1.  Erstellen Sie eine Duplikatkopie des Entwurfsdokuments, das wir ändern möchten,
    z. B. indem Sie `_OLD` zum Namen hinzufügen:
    `_design/fetch_OLD`. 
...
Um das Script `couchmigrate` verwenden zu können,
definieren Sie zunächst die URL der CouchDB/Cloudant-Instanz, indem Sie eine Umgebungsvariable namens `COUCH_URL` festlegen. 

_Definition der URL der Cloudant-Instanz:_
...
_Definition der URL der Cloudant-Instanz mit Authentifizierungsnachweisen:_
...
-   Beim Standardverhalten wird sichergestellt, dass der Index aktuell ist,
    d. h. dass er die aktuellen Dokumente in der Datenbank enthält,
    bevor die Antwort zurückgegeben wird.
    Wenn wir die Ansicht abfragen,
    indiziert Cloudant zuerst die 250 neuen Dokumente
...
-   Alternativ können Sie den Parameter `stale=ok` zum API-Aufruf hinzufügen.
    ...
    gibt Cloudant die Antwort sofort zurück,
...
-   Eine zweite Alternative ist das Hinzufügen des Parameters `stale=update_after` zum API-Aufruf.
    ...
    gibt Cloudant die Antwort sofort zurück und plant dann eine Hintergrundtask zum Indizieren der neuen Daten. 
...
>   **Hinweis**: Das Standardverhalten verteilt die Arbeitslast gleichmäßig auf die Knoten im Cloudant-Cluster. 
