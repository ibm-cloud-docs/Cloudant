...
Der kostenfreie [Lite-Plan](#lite-plan) enthält eine feste Durchsatzkapazität und Datenmenge für Entwicklungs- und Evaluierungszwecke. Außerdem wird ein kostenpflichtiger [Standardplan](#standard-plan) angeboten, der einen konfigurierbaren Bereitstellungsdurchsatz zur Verfügung stellt sowie eine skalierbare Preisstruktur für die Datenspeicherung, die an Ihre wechselnden Anwendungserfordernisse angepasst wird. Darüber hinaus wird ein optionaler [dedizierter Hardwareplan](#dedicated-hardware-plan)
mit einer zusätzlichen monatlichen Gebühr angeboten, um eine oder mehrere Ihrer Standardplaninstanzen in einer
dedizierten Hardwareumgebung auszuführen. Die dedizierte Hardwareumgebung steht Ihnen zur ausschließlichen Verwendung zur Verfügung. Wenn eine Instanz eines dedizierten Hardwareplans an einem Standort in den USA bereitgestellt wird, können Sie optional eine mit [HIPAA ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://en.wikipedia.org/wiki/Health_Insurance_Portability_and_Accountability_Act){:new_window} konforme Konfiguration auswählen.

In der folgenden Tabelle werden die bereitgestellte Durchsatzkapazität und der konfigurierte Plattenspeicherplatz für die verschiedenen Standardpläne aufgelistet:
...
<tr>
<th id="plans">Pläne</th><th id="lite">Lite</th><th id="standard" colspan='4'>Standard</th>
</tr>
<tr>
<th id="base-priceperhour" headers="plans">Grundpreis (pro Stunde)</td>
...
<tr>
<th id="provisionedthroughputcapacity-lookupspersecond" headers="plans">Bereitgestellte Durchsatzkapazität (Suchvorgänge pro Sekunde)</td>
...
<tr>
<th id="provisionedthroughputcapacity-writespersecond" headers="plans">Bereitgestellte Durchsatzkapazität (Schreibvorgänge pro Sekunde)</td>
...
<tr>
<th id="provisionedthroughputcapacity-queriespersecond" headers="plans">Bereitgestellte Durchsatzkapazität (Abfragen pro Sekunde)</td>
...
<tr>
<th headers="diskspaceincluded" headers="plans">Inbegriffener Plattenspeicherplatz</td>
<td headers="lite diskspaceincluded">1 GB</td>
<td headers="standard diskspaceincluded" colspan='4'>20 GB</td>
</tr>
<tr>
<th headers="diskoveragepergbhour" headers="plans">Plattenüberschreitung (pro GB/Stunde)</td>
...
Informationen zu den laufenden Kosten finden Sie in der [Preisstruktur](../offerings/pricing.html#pricing){:new_window}.
...
Sie können alternativ Ihre Standardplaninstanzen für eine monatliche Gebühr in einer dedizierten Hardwareumgebung in einem beliebigen [{{site.data.keyword.IBM}} Rechenzentrum ![Symbol für externen Link](../images/launch-glyph.svg "Symbol für externen Link")](https://www.ibm.com/cloud-computing/bluemix/data-centers) ausführen. Sie kaufen einen dedizierten Hardwareplan für einen bestimmten Standort und können
eine oder mehrere Standardplaninstanzen auf dieser Hardware bereitstellen. Dieser Plan ist für
die HIPAA-Konformität erforderlich und muss am Bereitstellungszeitpunkt ausgewählt werden. 

Der dedizierte Hardwareplan steht nicht für {{site.data.keyword.Bluemix_notm}} Dedicated-Kunden, sondern nur für {{site.data.keyword.Bluemix_notm}} Public-Kunden zur Verfügung.
{: tip}
...
Der Standardplan wird beispielsweise für 200 Suchvorgänge pro Sekunde bereitgestellt. Ihr Konto stellt möglicherweise während eines fortlaufenden Zeitraums von 1.000 Millisekunden (1 Sekunde) maximal 200 Suchanforderungen. Nachfolgende Suchanforderungen, die während des gleitenden 1.000-Millisekunden-Zeitraums gestellt werden,
...
Wenn Sie eine vorhandene Anwendung portieren, kann die Antwort `429` möglicherweise nicht verarbeitet werden. Als Teil der Migrationsverifizierung prüfen Sie, dass Ihre Anwendung
...
Um zu einer anderen Durchsatzkapazität zu wechseln,wählen Sie die gewünschte Bereitstellung aus und klicken Sie auf `Aktualisieren`.
...
bereitzustellen. {{site.data.keyword.cloudant_short_notm}} Daten werden nicht automatisch gesichert; es werden jedoch unterstützte Tools  
zum Erstellen von Sicherungen bereitgestellt. Im Leitfaden für
...
Sie können einen {{site.data.keyword.cloudant_short_notm}}-Lite-Plan oder eine Standardplaninstanz unter {{site.data.keyword.Bluemix}} auf zwei Arten bereitstellen:
