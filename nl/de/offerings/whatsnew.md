...
# Neuerungen in {{site.data.keyword.cloudant_short_notm}}

Bleiben Sie bei den Änderungen und Aktualisierungen für {{site.data.keyword.cloudantfull}} auf dem Laufenden.
{:shortdesc}
...
- Führt einen neuen Replikationsplaner ein.
  ...
  Kontaktieren Sie den {{site.data.keyword.cloudant_short_notm}}-Support, indem Sie im {{site.data.keyword.cloudant_short_notm}}-Dashboard ein Ticket öffnen, wenn Sie Fragen oder Probleme haben.
...
- {{site.data.keyword.cloudant_short_notm}} ist toleranter gegenüber fehlerhaften Benutzerdokumenten, die in der Datenbank `_users` gespeichert sind.
...
-	{{site.data.keyword.cloudant_short_notm}} Query gibt nicht länger einen Fehler zurück, wenn kein gültiger Index vorhanden ist.
...
-   Es ist ein Fehler, die Parameter `startkey` und `endkey` an eine Ansicht zu übergeben, wenn es nicht möglich ist, eine übereinstimmende Zeile zu finden. Wenn beispielsweise der Parameter `startkey` größer als der Parameter `endkey` für `descending=false` ist, oder wenn der Parameter `startkey` kleiner als der Parameter `endkey` für `descending=true` ist, gibt {{site.data.keyword.cloudant_short_notm}} den Fehler `400 Bad Request` zurück.
...
-   Validieren Sie die Struktur von Benutzerdokumenten, die in der Datenbank `_users` erstellt werden. Nach dem DBNext-Upgrade müssen die Benutzerdokumente ASF-CouchdDB-Anforderungen erfüllen. Früher hat {{site.data.keyword.cloudant_short_notm}} die Struktur von Benutzerdokumenten nicht validiert. 
