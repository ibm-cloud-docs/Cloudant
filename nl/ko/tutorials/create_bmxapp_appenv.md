...
# Cloudant 데이터베이스에 액세스하기 위한 간단한 Bluemix 애플리케이션 작성: 애플리케이션 환경

이 튜토리얼 섹션에서는 {{site.data.keyword.Bluemix}} 애플리케이션을 작성하기 위해 갖춰야 하는 애플리케이션 환경을 설정하는 방법을 설명합니다.
{:shortdesc}

<div id="creating"></div>

## Bluemix 애플리케이션 환경 작성

1.  {{site.data.keyword.Bluemix_notm}} 계정에 로그인하십시오. {{site.data.keyword.Bluemix_notm}} 대시보드는
...
3.  `Apps` 표제 아래에 있는 '`Cloud Foundry Apps`' 항목을 클릭하십시오. <br/>
    ...
    {{site.data.keyword.Bluemix_notm}}에서 사용 가능한 Cloud Foundry 애플리케이션의 목록이 표시됩니다. 
...
5.  '`Create a Cloud Foundry App`' 양식을 사용하여 Python Cloud Foundry 애플리케이션을 위한 환경을 지정하고 작성하십시오.
    '`Cloudant Python`'과 같이 애플리케이션의 이름을 입력하십시오. 호스트 이름은 자동으로 생성되지만, 이를 사용자 정의할 수도 있습니다. <br/>
    ![{{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 앱 이름](images/img0014.png)<br/>
    >   **참고**: 호스트 이름은 {{site.data.keyword.Bluemix_notm}} 도메인 내에서 고유해야 합니다. 이 예에서 도메인은 '`mybluemix.net`'이며 따라서 전체 호스트 이름은 '`Cloudant-Python.mybluemix.net`'입니다. 

6.  '`Create`'를 클릭하여 애플리케이션 환경을 작성하십시오. <br/>
    ![{{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 앱 작성](images/img0015.png)

7.  잠시 후 새 애플리케이션 환경에 대한 '`Getting Started`' 창이 표시됩니다.
    환경 내에 테스트 애플리케이션이 자동으로 작성됩니다. 애플리케이션은 자동으로 시작되며, 이는
    초록색 아이콘과 `Your app is running` 상태로 표시됩니다.
    이 애플리케이션은 일종의 '하트비트' 프로그램으로서, 새 애플리케이션 환경이
    사용할 준비가 되었음을 보여주기에 충분합니다. {{site.data.keyword.Bluemix_notm}} 계정 대시보드로 돌아가려면 '`Dashboard`' 링크를 클릭하십시오. <br/>
    ![처음으로 실행 중인 새 {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 앱](images/img0016.png)

8.  이제 대시보드에 새로 작성된 애플리케이션 환경이 포함됩니다. <br/>
    ![대시보드에 표시된 기본 {{site.data.keyword.Bluemix_notm}} Python Cloud Foundry 앱](images/img0017.png)

이제 사용할 준비가 된 {{site.data.keyword.Bluemix_notm}} Python 애플리케이션 환경을 확보했습니다. 

{{site.data.keyword.cloudant_short_notm}} 데이터베이스 인스턴스에 대해 작업하기 위한 다음 단계는 애플리케이션 환경과 데이터베이스 인스턴스 간에 '연결'을 작성하는 것입니다. 

<div id="connecting"></div>

## Bluemix 애플리케이션 및 서비스 연결

이 튜토리얼 섹션에서는 애플리케이션의 구성 및 관리 영역을 사용하여 {{site.data.keyword.Bluemix_notm}} 애플리케이션 환경 및 서비스를 연결하는 방법을 설명합니다. 

1.  {{site.data.keyword.Bluemix_notm}} 대시보드에서 애플리케이션 항목을 클릭하십시오. <br/>
    ![{{site.data.keyword.Bluemix_notm}} 대시보드에서 애플리케이션 선택](images/img0018.png)<br/>
...
2.  애플리케이션 환경을 다른 서비스에 연결하려면 '`Connections`' 링크를 클릭하십시오. <br/>
    ![{{site.data.keyword.Bluemix_notm}} 애플리케이션의 연결 구성 선택](images/img0019.png)<br/>
...
다음 단계는 {{site.data.keyword.Bluemix_notm}} 애플리케이션에 대해
작업하는 데 필요한 도구가 설치되어 있는지 확인하는 것입니다. 

<div id="toolkits"></div>

## Cloud Foundry 및 Bluemix 명령 툴킷

이 튜토리얼 섹션에서는 {{site.data.keyword.Bluemix_notm}} 환경, 애플리케이션 및 서비스에 대해 작업하기 위해 설치해야 하는 툴킷을 설명합니다. 

[Cloud Foundry ![외부 링크 아이콘](../images/launch-glyph.svg "외부 링크 아이콘")](https://en.wikipedia.org/wiki/Cloud_Foundry){:new_window}
툴킷은 Cloud Foundry 호환 환경에 배치된 애플리케이션에 대해 작업하는 데 필요한 도구의 집합입니다. 배치된 애플리케이션 업데이트, 또는 애플리케이션 시작 및 실행 중인 애플리케이션 중지와 같은 태스크에 이러한 도구를 사용하십시오. 

{{site.data.keyword.Bluemix_notm}} 툴킷은 {{site.data.keyword.Bluemix_notm}} 환경에서 호스팅되어 실행되는 애플리케이션에 대해 작업하는 데 필요한 추가 기능을 제공합니다. 

> **참고**: Cloud Foundry 및 {{site.data.keyword.Bluemix_notm}} 툴킷을 _둘 다_ 설치했는지 확인하십시오. 

툴킷의 다운로드 및 설치는 일회성 태스크입니다. 툴킷이 이미 설치되어 시스템에서 작동하고 있는 경우에는 이들이 업데이트된 경우를 제외하면 다시 다운로드할 필요가 없습니다. 

툴킷에 대한 일반 정보는 [여기 ![외부 링크 아이콘](../images/launch-glyph.svg "외부 링크 아이콘")](https://console.ng.bluemix.net/docs/cli/index.html){:new_window}에 있습니다. 

### Cloud Foundry 툴킷 설치

일부 운영 체제 배포판에는 사용 가능한 Cloud Foundry 툴킷 버전이 이미 포함되어 있습니다.
지원되는 버전이 6.11 이상인 경우에는 {{site.data.keyword.Bluemix_notm}}와 호환되며 이를 사용할 수 있습니다.
...
4.  <div id='checkCFversion'></div>작동하는 Cloud Foundry 툴킷이 있는지 확인하려면 프롬프트에서 다음 명령을 실행하십시오. 
    ...
    >   **참고**: Cloud Foundry 툴킷 도구는 {{site.data.keyword.Bluemix_notm}}와의 호환성을 위해 6.11 이상이어야 합니다. 

### Bluemix 툴킷 설치

시스템에 {{site.data.keyword.Bluemix_notm}} 툴킷을 다운로드하고 설치하려면 다음 단계를 사용하십시오. 

1.  {{site.data.keyword.Bluemix_notm}} 툴킷에 대한 링크는 애플리케이션의 '`Getting started`' 창에 있습니다. <br/>
    ![Cloud Foundry 툴킷 다운로드를 위한 링크](images/img0027.png)

2.  링크를 클릭하면 [다운로드 페이지 ![외부 링크 아이콘](../images/launch-glyph.svg "외부 링크 아이콘 ")](http://clis.ng.bluemix.net/ui/home.html){:new_window}로 이동됩니다. <br/>
    ![Bluemix 툴킷 다운로드 페이지](images/img0028.png)

3.  사용자의 시스템에 적절한 설치 프로그램을 다운로드하여 실행하십시오. 

    설치 프로그램은 Cloud Foundry 툴킷의 적절한 버전이 설치되어 있는지 확인합니다.
    모든 것이 올바른 경우 {{site.data.keyword.Bluemix_notm}} 툴킷이 시스템에 설치됩니다. 

4.  작동하는 {{site.data.keyword.Bluemix_notm}} 툴킷이 있는지 확인하려면 프롬프트에서 다음 명령을 실행하십시오. 
...
이제 {{site.data.keyword.Bluemix_notm}} 애플리케이션에 대해 작업하는 데 필요한 도구가 사용 가능합니다.
다음 단계는 {{site.data.keyword.Bluemix_notm}} 애플리케이션을 작성하는 데 도움을 주는 '스타터' 자료를 획득하는 것입니다. 

<div id="starter"></div>

## '스타터' 애플리케이션

이 튜토리얼 섹션에서는 {{site.data.keyword.Bluemix_notm}} 스타터 애플리케이션, 그리고 {{site.data.keyword.cloudant_short_notm}} 데이터베이스 인스턴스에 액세스하기 위해 이를 사용자 정의하는 방법을 설명합니다. 

{{site.data.keyword.Bluemix_notm}} 스타터 애플리케이션은 작동하는 {{site.data.keyword.Bluemix_notm}} 애플리케이션을 작성하는 데 필요한 최소한의 소스 및 구성 파일 집합입니다.
...
{{site.data.keyword.Bluemix_notm}} 스타터 애플리케이션은 {{site.data.keyword.Bluemix_notm}} 애플리케이션을 개발해 가면서 수정하거나 확장해야 하는 예제 파일의 아카이브입니다. 
...
'`Procfile`'에는 {{site.data.keyword.Bluemix_notm}}가 애플리케이션을 실행하는 데 필요한 세부사항이 포함되어 있습니다. 
...
{{site.data.keyword.Bluemix_notm}} Python 스타터 애플리케이션의 '`Procfile`'은 다음 예와 유사합니다. 
...
'`server.py`' 파일은 사용자의 애플리케이션에 맞춰 수정됩니다. 또는, 완전히 새로운 Python 소스 파일을 작성하십시오.
...
{{site.data.keyword.Bluemix_notm}} Python 스타터 애플리케이션의 파일은 다음 예와 유사합니다. 
...
-   '`domain`', '`name`' 및 '`host`' 값은 {{site.data.keyword.Bluemix_notm}} 애플리케이션 [작성](#creating) 시에 입력된 값에 해당합니다. 
-   '`name`' 값은 Cloud Foundry 툴킷에서 관리 중인 애플리케이션을 식별하는 데 사용됩니다. 
-   '`services`' 값은 '`Cloudant Service 2017`' {{site.data.keyword.cloudant_short_notm}} 데이터베이스 인스턴스가 애플리케이션 환경에 연결되었는지 확인합니다. 
