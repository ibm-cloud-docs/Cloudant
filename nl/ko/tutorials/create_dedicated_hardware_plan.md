...
2.  왼쪽 메뉴에서 `데이터베이스`를 클릭하고 데이터베이스 서비스 목록에서 `Cloudant`를 클릭하십시오.<br/>
     ![{{site.data.keyword.cloud_notm}} {{site.data.keyword.cloudant_short_notm}} 서비스 선택사항](images/img0003.png)<br/>

3.  페이지 맨 아래의 가격 플랜 목록에서 `Dedicated Hardware` 플랜을 선택하십시오.<br/>
    ![가격 플랜](/docs/services/Cloudant/tutorials/images/pricing_plan.png)
    
4.  페이지 맨 위에서 다음 매개변수를 기입하십시오. <br/>
    -   서비스 이름을 입력하십시오.<br/>
    ...
    ![데디케이티드 하드웨어 구성](/docs/services/Cloudant/tutorials/images/select_deployment_location.png)
...
5.  `Create` 단추를 클릭하여 프로비저닝 프로세스를 시작하십시오. <br/>
    ...
    ![데디케이티드 하드웨어 프로비저닝](/docs/services/Cloudant/tutorials/images/create_button_provision.png)
    ...
    ![데디케이티드 하드웨어 대체 버전 프로비저닝](/docs/services/Cloudant/tutorials/images/create_button_provision2.png)
...
3.  가격 플랜에서 `Standard`를 클릭하십시오. <br/>
    ![표준 가격 플랜](/docs/services/Cloudant/tutorials/images/standard_pricing_plan.png)
...
4.  다음 매개변수를 채우십시오. <br/>
    ...
    -   인증 방법을 선택하십시오.</br>
    -   환경을 선택하십시오.</br>
    ![표준 인스턴스 구성](/docs/services/Cloudant/tutorials/images/select_environment.png)
    
5.  `Create` 단추를 클릭하십시오.<br/>
몇 초 후에 선택한 환경에 인스턴스가 프로비저닝됩니다.<br/>
    ![표준 인스턴스 프로비저닝](/docs/services/Cloudant/tutorials/images/create_button_provision_standard.png)
...
`legacyCredentials` | 인스턴스가 레거시 및 IAM 인증 정보를 둘 다 사용하는지 또는 IAM 인증 정보만 사용하는지 결정하는, 기본값이 true인 선택적 매개변수입니다. 인증 방법 선택에 대한 세부사항은 [IAM 안내서 ![외부 링크 아이콘](../images/launch-glyph.svg "외부 링크 아이콘")](/docs/services/Cloudant?topic=cloudant-ibm-cloud-identity-and-access-management-iam-#ibm-cloud-identity-and-access-management-iam-){: new_window}를 참조하십시오.
...
서비스 인증 정보에 포함된 필드에 대한 자세한 정보는 [IAM 안내서 ![외부 링크 아이콘](../images/launch-glyph.svg "외부 링크 아이콘")](/docs/services/Cloudant?topic=cloudant-ibm-cloud-identity-and-access-management-iam-#ibm-cloud-identity-and-access-management-iam-){: new_window}를 참조하십시오.
...
`enable-internal-service-endpoint` | 내부 엔드포인트로 서비스 인증 정보의 URL 필드를 채워 {{site.data.keyword.cloud_notm}} 내부 네트워크를 통해 서비스에 연결하기 위한 선택적 필드입니다. 공개적으로 액세스할 수 있는 외부 엔드포인트로 URL을 채우려면 이 필드를 생략하십시오. 내부 엔드포인트를 지원하는 데디케이티드 하드웨어 환경에 배치된 표준 플랜 인스턴스에만 적용됩니다. 환경이 내부 엔드포인트를 지원하지 않는 경우 명령으로 인해 400 오류가 발생합니다. 
