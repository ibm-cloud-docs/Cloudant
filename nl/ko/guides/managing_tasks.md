...
[`_active_tasks` 엔드포인트](/docs/services/Cloudant?topic=cloudant-active-tasks#active-tasks)는 모든 진행 중인 태스크에 대한 정보를 제공합니다.
...
인덱싱 태스크 완료 시간 예상이 100% 정확할 수는 없습니다. 태스크를 완료하는 데 소요되는 실제 시간은 다음과 같은 요소에 따라 달라집니다.
...
정지된 복제를 해결하려면 [복제 프로세스를 취소](/docs/services/Cloudant?topic=cloudant-replication-api#canceling-a-replication)하고 다시 시작해야 합니다.

이렇게 해도 해결되지 않는 경우에는 소스 또는 대상 데이터베이스에 액세스 중인 사용자에게 쓰기 권한이 없어 복제가 정지되었을 가능성이 높습니다.

복제에서는 [체크포인트](/docs/services/Cloudant?topic=cloudant-replication-guide#checkpoints)를 사용합니다. 이는 복제가 다시 시작되면 이미 복제되고 변경되지 않은 컨텐츠를 다시 복제할 필요가 없음을 의미합니다.
{: note}
