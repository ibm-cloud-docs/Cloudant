...
{{site.data.keyword.cloudant_short_notm}}는 [`_changes` 엔드포인트](../api/database.html#get-changes)로부터의 단일 HTTP 피드를 통해 데이터베이스에 영향을 주는 추가, 편집 및 삭제 항목을 공개합니다.
...
고급 복제 API 옵션은 [여기](../api/advanced_replication.html)에서 사용 가능하며, 복제 중에 사용되는 컴퓨팅 성능을 높이거나 낮출 수 있습니다.
예를 들면 다음과 같습니다.
