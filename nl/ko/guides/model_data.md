...
{{site.data.keyword.cloudantfull}}에서 데이터를 모델링하는 방식은 애플리케이션의 스케일링 능력에 크게 영향을 줍니다.
...
최우선 개정판은 문서의 전체 트리에 속한 하나의 특정 개정판입니다. {{site.data.keyword.cloudant_short_notm}}의 문서는 사실 개정판의 트리라는 점을 생각해 보십시오.
문서에 대한 요청이 이뤄지면 임의의 결정론적 알고리즘이 이 트리에서 하나의 삭제되지 않은 리프를 선택합니다. 많은 분기 요소가 있는 큰 트리는 분기가 없거나 적은 트리보다 문서를 처리하는 데 오랜 시간이 소요됩니다.
