...
O comportamento padrão distribui a carga uniformemente entre os nós no cluster do {{site.data.keyword.cloudant_short_notm}}. Se você usar as opções alternativas `stale=ok` ou `stale=update_after`,
