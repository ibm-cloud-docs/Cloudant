...
_Example of using HTTP to retrieve a list of the first five documents from
the `recipes` partition of a database, applying the user-created `by_title` view:_
...
_Example of using the command line to retrieve a list of the first five
documents from the `recipes` partition of a database, applying the user-created
`by_title` view:_
...
View index rebuilds occur whenever a change occurs to any one view from all the views that are defined in the design document. For example, if you have a design document with three views, and you update the design document, all three view indexes within the design document are rebuilt. If you need to make changes to a design document for a larger database,
...
To do this, use the `update` query string parameter when making a view query.
...
### Query freshness control options
...
_Example showing that the global view query returns no entries, because `alpha` is alphabetically before `beta`,
...
_Example that uses the command line to apply correct filtering and sorting
to a global query:_
