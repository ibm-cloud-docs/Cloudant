...
> **Note**:  Within a search index, do not index the same field name with more than one data type. 
If the same field name is indexed with different data types in the same search index function, you might see 
an error when you query the search index that says the field, "was indexed without position data". 
For example, you might get this error if you include both of the following lines in the same search index function. 
Both lines index the same 
field `myfield` as two different data types: a string `this is a string` and a number `123`.
```json
